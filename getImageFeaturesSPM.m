...
%   h: histogram of visual words of size {dictionarySize * (4^layerNum - 1)/3} (l1-normalized, ie. sum(h(:)) == 1)
    ...
    % for layer 1 to N
    weight = zeros(3, 1);
    L = layerNum - 1;   % L = 2 when layerNum = 3
    weight(1) = 2^(-L);
    for i = 0:L-1
        weight(i+2) = 2^(i - L);
    end
    ...
    for i = layerNum:-1:1     % i = 1:3
        his{i} = zeros(2^(i-1), 2^(i-1), dictionarySize);
        % compute the histograms from the finest layer
        if i == layerNum
            for j = 1:2^(i-1)
                for k = 1:2^(i-1)
...
%             his{i}(:, :, :) = his{i}(:, :, :) / sum(his{i}(:));
        % aggregate the histograms from the previous layer            
        else
            for j = 1:2^(i-1)
                for k = 1:2^(i-1)
